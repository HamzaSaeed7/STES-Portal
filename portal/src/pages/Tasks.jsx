...
function CreateTaskModal({ open, onClose }) {
  ...
  return (
    <Modal open={open} onCancel={onClose} footer={null} centered width={440}
      title={<span style={{ color: '#111827', fontWeight: 700 }}>Create Task</span>}
    >
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 16 }}>
                <div style={{ flex: 1 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6 }}>
                    <Text style={{ color: '#111827', fontWeight: 600, fontSize: 14 }}>{task.title}</Text>
                    <Tag color={cfg.color} icon={cfg.icon} style={{ borderRadius: 6, fontSize: 11 }}>
                      {task.status}
                    </Tag>
                  </div>
                  <Text style={{ color: '#9ca3af', fontSize: 12, lineHeight: 1.6 }}>{task.description}</Text>
                </div>
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 6, flexShrink: 0 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    ...
                    <Text style={{ color: '#6b7280', fontSize: 12 }}>{task.assignedTo}</Text>
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
                    <CalendarOutlined style={{ color: '#9ca3af', fontSize: 11 }} />
                    <Text style={{ color: '#9ca3af', fontSize: 11 }}>{task.createdAt}</Text>
                  </div>
                </div>
              </div>
            </Card>
